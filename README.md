...
And now the last piece of the puzzle is that we just have to have one more Terraform file change here. We’re going to make a new Terraform configuration file called backend. Remember, this could also just go in main.tf — there’s no real significance to having different file names. But the reason we’re having this separate file is that we want to tell Terraform that for its backend, it should be using S3.

So, we go into our Terraform directory, and we’ll make a new file — call it backend.tf. There it is. Paste that in and save. And that’s there. So, and this — this is not a temporary file. This is for real. Terraform needs this to tell it to use S3 as its backend.

So, we set up a couple of temporary Terraform files — one of them to allow us to build some useful S3 buckets and DynamoDB tables for lock. And then we created one to set up the appropriate permissions — the role and policies. We ran both of them one time to set up all that infrastructure. We deleted those Terraform files, and we’re done. We’ve set that up.

And now we’ve just created this backend Terraform configuration right here, and we’re almost ready for prime time. This is all, of course, just a one-time setup. You wouldn’t ever have to do this again. Once you do it for any repo, it’s done — so that it’s then properly hooked up to GitHub Actions.

All right, back we go to the instructions. The next step is — it’s time to add some secrets to GitHub. GitHub has to know the secrets that we’re going to use. Let’s go and do that right now.

Okay, so we’re back in GitHub. We have gone to our twin repo — here it is. We go to Settings in the top navigation, scroll down to Secrets and variables, and then go to Actions. Here we go — these are the secrets and variables that apply to GitHub Actions. We want a repository secret — a secret that applies to the whole repo. Press New repository secret.

All right, what secrets do we want? The first one is AWS_ROLE_ARN. AWS role ARN tells GitHub what role it should have in AWS. And there, in my clipboard, is the secret — the one that we just took from running that Terraform thing once. This is the value of the ARN that it should use for GitHub Actions to deploy the twin. Add the secret — everything has to be exactly right, don’t forget that.

Okay, now what’s the next secret? The next secret is DEFAULT_AWS_REGION. Well, that’s perfectly straightforward — no mistakes. It’s got to be your default region. It’s best to copy and paste it from somewhere rather than typing it, like I did, to make absolutely sure. Double-check, because if you get something wrong, it’s really hard to find out what went wrong later — you may get some obscure error.

All right, and then the AWS_ACCOUNT_ID — that’s the final secret we need to add here. So, back we go here and we go to New repository secret and enter AWS_ACCOUNT_ID. And I can cheat — I can paste in the ARN from before because my account ID is in there. Look at that — there we go, a nice little pro hack that is. The AWS account ID for me — and you should put in yours — and press Add secret.

And now we have three secrets: AWS_ACCOUNT_ID, AWS_ROLE_ARN, and DEFAULT_AWS_REGION, and they should all be good. And that has set us up for success.

Now it’s time to create the workflows. Okay, so as it says, at this point, GitHub now has the ability — it has the power — to log into our AWS account with the credentials that we set up for it. It’s all ready to actually do something.

So, over here in twin — let’s collapse some of these folders so it looks nice and clean — we need to create a new top-level folder called .github. And within .github, we’re going to have a subfolder called workflows. New folder — workflows. There we go. Somewhat confusingly, Cursor shows it this way when there’s nothing else but workflows, but you can actually turn that off if you wish.

Anyways, we’re now going to create our first workflow, and it’s going to be called deploy.yaml. So here we right-click, choose New File, and name it deploy.yaml — without the “A,” just like that. And now we’re going back to the guide to get our deployment script.

So, this is what a GitHub Actions deployment script looks like. And I’m going to take all of this — you can see there’s quite a lot to it. Copy that, and now I’m going to go back to deploy.yaml and paste it in. Here it is. This is our first look at a GitHub Actions deployment script. We finally got to this point — we’ve set everything up. We’ve set up the backend S3 buckets and DynamoDB, we’ve set up the policy to log in, and this is now what will be run.

So what does it look like? What does the script do? Well, it begins with a name — deploy digital twin. It says on, and this has a description of the event — what will cause this to be triggered. And it’s on push. It says which branch, and it says what to do about it.

It gives various information about permissions. And then we’ve got the jobs — remember, jobs were the key ingredient. A job has a number of steps. We’ve got several steps under one job — the job is called deploy.

And that one job has several steps: check out the code, set up Python, install dependencies, set up Terraform, set up Node.js, and then look at this step here — run the deployment script. It sets some environment variables — the account ID and the default AWS region — which it gets from its secrets. It makes the deploy script executable — the same deploy script that is already in our scripts folder.

The Mac version, because it’s a Mac and Linux version — and this is why, PC people, I told you that you had to keep the Mac version of the deployment scripts, because even though you only run the PC version locally for you, when GitHub runs it, it uses the Mac/Linux version.

It also uses the AWS role ARN so that it has the right role. And we then get the outputs from that. You can see everything that’s going on in the script by looking through these steps — spend some time on it. YAML means that it’s very human-readable, so it’s very clear and easy to see what’s going on, and you can understand the script.

One more thing I have to do though — I can see from that white blob there that I haven’t saved it yet, so I need to make sure I save it before we go on to our first GitHub workflow.

But we have a second one to talk about too — we’ve got one called destroy. We’re going to set up a destroy.yaml which will destroy an environment. Let me just drag all of this, select that, and copy it in this directory here. New file — destroy.yaml. There it is. Paste in the contents, remember to save, and let’s have a look at it. It doesn’t have any automated reasons.

# **AD) Day 5 - Live CI/CD Pipeline Deploy: From Git Push to Production AI Agent**

Okay, let’s do this. So, I bring up a new terminal. I’m nervous, I’ve got to admit — we’ve done quite a lot. Uh, here we go — new terminal. So, git status tells us the situation. We’ve got these two deploy scripts, two destroy scripts, and we’ve got the Terraform backend that we’ve added as well. It doesn’t show us here, but we’ve also got the GitHub part of it too. So let’s do git add. It doesn’t show us — sorry, because it’s just showing us the whole directory there. When we do git add ., now we do a git status, we should see everything. Here it is. We’ve got the destroy and deploy workflow ready to go, and then all of our updates to the scripts and backend.

git commit -m — could be famous last words. It may not work first time. We’ll see. And then git push, and we’ll see what happens. Off it goes. Let’s go over to GitHub. So, here we are in GitHub again. We come up and now we go to Actions at the top here, press there, and right away we see that there’s a workflow that is running called Add CI/CD with GitHub Actions. It’s yellow and it’s running. You can see on the top left here there are two workflows — Deploy Digital Twin and Destroy Environment.

If I click on that first workflow, we can see right here that it is running. And if I click in on this, then we come in here and we see it’s been running for 50–52 seconds now, deploying to dev. And, uh, things are happening, and we’re hoping that this is going to complete and our twin will have been deployed. Remember, we destroyed all infrastructure, so there’s no digital twin right now. And it is hopefully deploying as we speak. Click into it — we can actually see what’s going on, we can watch it happening. It’s, uh, no surprise, of course, it’s in the CloudFront distribution part.

Let’s just scroll up and see what’s been happening before this point. We see in here — running the deployment script — we see all of the different tasks. It checked out the code, configured the AWS credentials, set up Python, installed UV, set up Terraform, set up Node. It’s currently running the deployment script. Here you can see all of it going. Uh, this is super familiar — I hope this is the part where it builds the Lambda package and then zips it up, creating the Lambda deployment package, creating the zip file, initializing the backend S3. And, uh, yes, you can see all that’s happening.

I’m obviously not going to be able to keep this running commentary going for the full five minutes that it takes to set up the CloudFront distribution, so I’ll do the usual thing of going to the future. But this is so exciting — I’m going to be riveted looking at this and hoping for a home run and a first-time deploy live. Fingers crossed. Let’s see.

So, I can’t resist giving you, like, the live-action commentary. It just completed CloudFront. It’s going super fast. Uh, and it’s just, um, it’s now — because it’s now done the deployment — it’s got the bucket name and the memory bucket. But what it now needs to do is, of course, that final step of now building the front-end package and deploying that with the right variables so that it has the right fetch URL, so that the front-end fetches the right back-end. So that’s what it’s just compiled — the Next.js page — while I watch it. This will run faster than I can speak. It’s just finished that, it got the deployment, it invalidated CloudFront so that it refreshes with the very latest, and it’s just saying as the deployment summary: deployment complete — CloudFront URL, API Gateway, front-end bucket — and it’s all finished successfully. Complete job.

So, I’ve been following along; you obviously don’t need to. What you may well have done is just have gone to this job, and you’ll have seen this list of tasks that all ran successfully. The biggest one, of course, was the five minutes spent mostly configuring CloudFront. And what you can simply do is open up this deployment summary here, and you’ll see a little report of where to go. And this tells you where we will find our running deployment. Let’s try that now.

So here we go — I’m going to press a link on the CloudFront URL. And up comes a digital twin. Uh. Hi there. Does it work? Fingers crossed. Is it going to get to our back end? Is it going to call AWS? And there we go. It worked. It worked! “Do you like cheese?” Why not? Do you like cheese? Let’s see. Ah, there we go — we get a good answer. So — wow! Ha! It just worked again. This is very sophisticated. And if this is the first time that you’ve built a CI/CD pipeline, you might be thinking — this was an awful lot of work. It was a huge amount of configuration and faffing around with that Terraform stuff and AWS permissions.

But you may not appreciate, if you’re not from a DevOps background or you haven’t gone through and set up something like a Jenkins pipeline before, you might not know how grueling this can be. This can be weeks of work to set up a reliable process that can build infrastructure automatically and deploy to dev like this on doing a git push. That kind of thing is really, really hard. And so the fact that we got all of this done during one day is, in fact, fantastic. You should be blown away by — dare I say it — how easy this was. It probably didn’t feel that easy, but we did get it all done and it worked.

And if yours isn’t working, got some error — then roll up your sleeves. I imagine there’s going to be just something somewhere so small — any tiny typo can cause a problem, and they can be quite hard to track down. So that will be something for you. But I’m hoping that you’ve had the same experience as me here — and it just worked, and it’s set up, and it’s deployed. And that is really, really cool.

And just to show you one more time — if we go back to GitHub Actions again, I’m just going to go right back. If, supposing that we were looking at our code right here, we could click on Actions. This brings up GitHub Actions. Here you see the workflow run that we just did — you can see that it’s my only run and it was successful, first time, beginner’s luck. And I’m going to click on Deploy Digital Twin over here to look at this workflow. Here it is — it ran nine minutes ago. You can click in on it. There it is — deploy to dev. That took five minutes and 29 seconds. You click into this, you see the different steps. You come down to Deployment Summary — this is where you go to get your URL for this deployment.

Okay, congratulations! And we will do a little bit more testing. The next thing to show you is that we just deployed to dev by just doing a git push. Just doing a git push, and off it went to dev and built the infrastructure — and it’s a go. You might be wondering, okay, how about — how do we do test and prod? Well, we’ve got that set up. So you have to do that manually. You don’t have to do anything manual — you just have to trigger it manually, not on a git push.

But of course, you can set things up in GitHub Actions — you can do clever stuff like when someone accepts a PR review or does something, it can all happen automatically. But for now, we are going to do it manually. And the way that you trigger that is that we go to GitHub Actions, over here we go to Deploy Digital Twin. This workflow, over on the right here, has a button Run workflow. We press that dropdown, we say that we want to deploy to test. You see how easy this is — and we press Run workflow. And that is now going to run a deployment to test.

And one of the things that maybe you’re wondering, I don’t know, is — okay, but basically the first time we run this, we want it to create all of the infrastructure; subsequent times we just want it to deploy. How is that going to work? And the answer is — that works by the magic of Terraform. Terraform automatically has the state — it knows the resources that are there, and Terraform just handles not recreating things when they already exist. So that is just something that we trust with Terraform. That’s what it’s all about — that idea. Not only is it good at creating and destroying infrastructure, but it’s also good at maintaining state. So you can just run the Terraform script again, and it knows not to duplicate infrastructure. That’s what it’s all about.

So we’re now running the Deploy Digital Twin to the test environment. And this time I won’t get overexcited and give you a running commentary. I will come back in a minute — or in five minutes — when this has deployed, so we can check it out.

And here we are, seeing a green tick by Deploy Digital Twin — so two runs, two green ticks. This is looking good. We go into here, we click on Deploy to Test, we come down to the deployment summary step — deployment complete. Here’s the CloudFront URL. Ready? Drum roll. Click on this. Up it comes. Uh. Hi there. And away we go. It’s working. It’s working. I’m very happy to see that indeed.

Okay. And now back to Deploy Digital Twin — it’s time for us to kick off our production deployment. So back we go to Actions, and then go to Deploy Digital Twin. And then over here, we come to Run workflow, and we’re going to choose Production (prod). And we press Run workflow, and off it goes. Any second now we should see our new workflow running. Is it going to be a hat trick? Am I going to have three for three — all successful first time? We’ll soon find out. There it goes — off it starts. I will leave you again — five minutes for me, a couple of seconds for you — and we’ll see if this completes as well.

Okay, here we go — three for three. Am I daring things? Not even going to go in there. I’m just going to open up a new tab and type Digital Twin, and here we are. Here we have it. Wow! This is our digital twin live on the internet. Let’s say “Hi there.” Check — before I declare victory too early, let’s hope we get back a response. Now, serve me right if we get some failure — but, well no, we get a very meaty response indeed.

There we go — we have our AI in production, deployed three for three. That’s really cool. I hope you’re having the same experience. I hope that you, now, through the power of GitHub Actions, have been able to deploy your latest code to dev, test, and production. And you, like me, have three apps running here, with one of them being on a public URL. Congratulations!

# **AE) Day 5 - Automated CI/CD Pipelines for Production AI Apps with Git Deploy**

And another thing—just one more thing to wrap this up. Let’s make a change to the code. Let’s change something in the code, do a git push, and see it all happen automatically. Let’s make our digital twin a bit better, and let’s start by adding a little profile picture, an avatar, to appear instead of that little robot on the left. It’s a tiny thing, but it’s kind of fun. So, if you look in the front-end of the twin project—which is now a repo—look at the public folder. We’re going to drop in this folder a new file called “avatar,” which should be your favorite little profile picture of yourself. I’ve dropped one right here. I don’t have a PNG, I’ve got a JPEG, so I’m going to make it “avatar.jpeg.” I’m dropping that in the public folder. Move—there we go. You can copy and paste it from your file system as avatar.jpeg.

Now we’re going to update the twin component, and we’re actually going to make two changes to the twin component points. We’re going to update it to look for avatar.jpeg, and we’re also going to fix that tiresome problem that has probably been irritating you—that when you’re chatting with your twin, every time you press Enter, that field loses focus and you have to click back in it again to send your next message. Infuriating! And of course, we should fix it. I mentioned we were going to fix it, and you might have been thinking I forgot about that because it was ages ago, but I didn’t forget. This is what I had in store for you—fixing it when we do the git push for an auto-deploy.

All right. So we’re going to select this right here, this new version. Here it is. It’s basically the same, just with those couple of extra parts in it. So I’m now going to go back—it feels like such a long time ago—we were in twin.tsx. Select all, paste, and save. And now all I have to do myself is just look for “avatar.png” and replace it with “avatar.jpeg.” So let’s do that. I will even change the comments to JPEG. You don’t need to do this if you have a PNG file. Where next? Here—and where next? Here. Oopsie, there we go. Selected everything—JPEG. And one more—yeah, there we go. JPEG. Done. No more results. Save. All right. It’s now avatar.jpeg for me, still PNG for you, I hope. And that is our change.

Okay, back to the guide. Let’s see what we’re meant to do next. We’re just meant to commit and push the fix. There’s one other change I want to make as well. Have you noticed it looks a bit janky that when we bring up our digital twin, the title says something like “Create Next App” or something? It just looks a little bit amateur. And the favicon—the little icon in the browser tab—is like a standard Next.js icon. We should fix those things right now. It’s actually super easy to fix. We just come up to the app folder, and you’ll see in there a favicon icon, which is that Next.js logo. We want to get rid of that, so we just right-click and delete it, move to trash.

Then there’s also a layout page which has within it the title “Create Next App.” Let’s change that to “Ad Donna’s Digital Twin.” There we go. That will do. Description: “An AI career twin for Ad Donna, representing Ad Donna.” Okay, that seems good. I’ve also got my favicon right here—I just made a little favicon. If you don’t know how to make them, just Google it; there are endless tools for this. And I just have to drag that in. Let’s collapse some of these other things that are holding me up here. Collapse that. And where does it need to go? It needs to go in the app page. Here it is—favicon goes in the app folder. There it goes. Move—in it goes.

I should mention that when we deploy this, favicons get cached by the browser, so this little extra piece might not actually appear immediately. But anyway, that’s another change for us to make. So those are a few minor enhancements.

What do we do next? Well, do we have to do so much work to go and prepare everything for deployment? No, of course not—it’s going to be very simple. Git status—there are the files we’ve changed. We’ve added the favicon, changed the layout, changed twin.tsx, and we put an avatar in it, in public. Right—that’s where we were told to put it. Yes, okay, good. So now we do git add ., git status again—always worth checking what we see. There are our four changes ready to be made. Git commit: “Various UI refinements.” And we do a git push—and that should be it. Git push—there we go. Off it goes, somewhere on the internet.

GitHub is whirring up and it’s setting up its VM, it’s running the workflow, it’s going to be looking in our S3 buckets for Terraform states, that’s going to be hitting AWS—everything will be happening. Let’s go and take a look. I’ve just quickly gone over to GitHub. I’ve gone to Actions. We can see that this is busily running: “Various UI refinements.” It’s happening—deploying to dev. It’s been going for a minute. It’s currently—no doubt—where are we? It’s moving pretty quickly. It’s in our favorite place—no surprise, shockingly. And I will let this do its thing, and I’ll be right back with you.

Well, here we are, back at GitHub Actions again, and we’re four-for-four for green ticks. That’s what I like to see. Okay, so now we go into this workflow into “Deploy to Dev.” We scroll down, we go to Deployment Summary, and we have a CloudFront URL right here. We’ll add it in here. The first thing you notice right away—Adna’s digital twin. We’ve got a different title to the others. As I warned you, the favicon hasn’t updated because browsers cache them, but hopefully that will at some point update. But it’s cool to see that change immediately there.

Hello! I’m interested to see that avatar—were you expecting that as well? And now I’ll say “Hi there,” and we press send that in, and you can see my little avatar there as I’m thinking. And you can see we’ve got focus back there—no more need to click any more. Our UI refinements have been deployed. It worked perfectly, flawlessly, four times out of four. You’ve just seen how easy it is—how incredibly easy it is—to make some changes to the app, do a git push, and bam! They’re deployed live on the internet, available publicly. I would say that that is success.

# **AF) Day 5 - Resource Management and Cost Control for Production AI Systems**

And so there’s a few activities for us to do to wrap up, after this amazing week and a huge amount of infrastructure we’ve built. First of all, you should yourself go in as your IAM user and look at CloudWatch and use that as a way to track things like the calls, the invocations of Nova through Bedrock, and look at the logs, look at the Lambda logs, satisfy yourself of everything that’s going on across your different environments. We showed how to do that before, so you should now be an expert at that. And so, go do that.

And then what I want to share with you is another AWS thing for which we need to go in as the root user. I have logged in here as the root user. Look at the top right—you can see it’s my name. We are indeed in as the root. And I’m going to search now for something new, a new one we’re going to look at which is going to be the AWS Resource Explorer. This is something we can do to get an overview of all of the different AWS resources that we currently have running. And I’m going to type “twin” so that we can have a look at all of the different twin-related resources. You can see there’s some for tests, there’s some for prod, there’s some for dev, there’s S3 buckets, there’s all sorts of stuff going on, lots of different things that we have running related to our twin.

And with that, the next thing in store for us is to get rid of them. Let’s destroy them all. We could go through this console and spend an awfully long time destroying them all, or we could use that other convenient GitHub workflow that we set up, which just calls our destroy script that we wrote before. Let’s do that. So back we are in GitHub Actions, and we’re going to go to—this is all of our workflows that we’re seeing right here—we’re going to go to the destroy environment workflow. Here it is. It’s not been run yet. We’re going to click here, and we’re going to start by destroying the dev environment. It’s one of these tabs here that’s running. Let’s look at that. This environment destroy—we have to type the environment name again to destroy it, a little check that we’ve got in there—clicking to run the workflow. Off it goes. In a second, this screen should refresh to show that it’s running, and when it does so, I’ll click in on it so we can watch it anxiously.

There it is. Click on it. Here it goes. Click in on it again to see the steps that are going on. It’s running the destroy script right now. Look at it. I’m not touching anything. If you think that I’m running this, this is machinery in motion. And as usual, it’s going to get hung up on CloudFront, which is where it takes time. Look at all that—so much stuff happening—destruction complete and that all finished. And it does say environment dev has been destroyed. That’s another successful job for us. Okay, so back to GitHub Actions to destroy. Look at that—now we’re five greens. Can I make this a complete home run? Next up, time to destroy the test environment. Test. Test. Run workflow. And off we go—destroying test. See you in a second.

And you’re probably getting bored of this by now, but sure enough, the second one ran successfully. So now we just come on in, we do run workflow. Now it’s time for us to kick off the production destruction—prod. We type prod again to confirm the destruction, press the workflow button, let it do its thinking, and I will see you back in a minute. And there we have it. Production also destroyed—three successful destroys run for dev and test and prod. And so if we go to all of our workflows here, we have the three original deploys, the UI refinements push, and the three destroys—all looking good. And presumably, if we now come over here and try and hit this, we get a page cannot be displayed. We try and hit this—page can’t be displayed—and page cannot be displayed. And you get the message: page cannot be displayed. We have success.

And now if we go back to AWS in as the root user, go into the AWS Resource Explorer and search again for twin, I think it was like 35 or something resources before—there’s now 13 resources. There’s still some resources. Why? Why is that, you ask? Well, unless you already know why that is. First of all, there’s some logs here, and logs may hang around. That seems like a fine thing. And then the other stuff is the DynamoDB with the Terraform locks and the role that we set up and the S3 bucket. This is the kind of one-time, not associated with any environment, but set up for GitHub Actions, and we wouldn’t want to delete that. So that is all still here along with this thing here—it’s from a different setup I did. You can ignore that one. And there’s also that dashboard that I set up.

Now these Terraform states—they do cost something. If you’re not on your free period, then there is a tiny, tiny charge associated with them. So should you be completely done with the twin project, you can go out and delete all of these as well. But it is really very, very small. But otherwise, you can see for sure we’re down to the bare bones. We’re down to like the configuration stuff. The actual environments—the real Lambda, S3, CloudFront, and API Gateways associated with our three environments—have all been completely destroyed. And the Resource Explorer is the right place to do this, to find it out.

And just before we leave the AWS console, there’s something else that we should do, that you should always do every so often—which is come in and look at Billing and Cost Management. Billing and Cost Management. And my numbers are probably ticking up for various other reasons, but yours should not be. You should come into this, you should see your costs, you should see a little chart here of what you’re spending money on, and you should be absolutely sure that it’s what you expect, that everything is looking good. You can always come in and analyze your costs. You can see what’s costing what, if anything is costing anything, and if you have anything that is costing that you’re not expecting, then go in and delete it. And that is part of your healthy cost management and budget management that you should do on a regular basis.

Okay. With that, that really does conclude the lab for today. It was quite a long lab, but wow—we got a lot done. Let’s go back for the wrap-up. Now look, I’ve got to come clean with you. I hope that you got six green checks right there and you have had full success first time round, and I made out as if I did too. But as you probably guessed, that’s not entirely accurate. Whilst it is true I did have six greens there, I’m not perhaps completely revealing the fact that this is not my first time running this particular project. But in preparation for this course, I of course set this up and ran it a couple of times. And believe me, I’ve had my fair share of reds. And I have butted my head against the wall, as you probably have had to too. But I was writing all of that code the first time and made plenty of mistakes along the way.

So it is—it’s something—it’s an experience. You get better and better at it. That’s where the real learning happens. So if you’ve been cursing—hopefully not cursing me—but cursing Terraform and GitHub Actions all the last few days, then that’s par for the course. That’s what it takes. And as a result, you now have deeper expertise than your friends who happened to get lucky and have green first time. You did better than them. And I should also reveal that while I’ve talked about this week as if it’s a one-week part of the course, it took me way longer than a week to get everything working properly. And so this is definitely—it feels like this whole course every week is really beefy, and we got so much done. And if it took you a lot longer than a week, then I’m not surprised. I think it’s a week if you really did very little else for a week. There’s a lot packed in.

But anyways, we built a digital twin app with memory in day one after talking about different architectures. We then deployed it to AWS using the AWS console—that involved building a serverless architecture using Lambda, S3, API Gateway, and CloudFront. And we wrote Terraform scripts to create and destroy three different mirror environments with the production environment. We also used a registered domain name and set up the DNS configuration associated with that, including SSL certs. And then we automated everything with GitHub Actions so that we can git push to deploy, we can promote between environments, and we can destroy environments. That is what we got done this week. Wow.

Oh, and yes—we also were using Amazon Bedrock and we connected with Amazon’s Nova foundation models. There was some AI. There was some AI in this. So, as before, to make the point that putting AI into production has AI involved through an AI lens, but a lot of the work is core platform engineering stuff. But hopefully I’ve taken you through this always with that AI lens—like the conversation history as well.

And this brings me to the assignment for you. So, some of you may have taken my AI course and already built a digital twin that had nothing like the production capability—it was a Gradio app—but what it did have was things like the use of tools to be smarter. So if the person asks a question that it doesn’t know, it’s able to look it up and record—it’s able to look up and record the fact that it didn’t know the answer, it’s able to send a push notification, and we also record some of the details. And my suggestion for you is—now is your opportunity to go and beef up this LLM, this application. Whether or not you’ve taken the Agentic course, you should still be able to dig in and get this done. Add in some tools—perhaps use an S3 bucket to record questions that the LLM doesn’t know the answers to, and to look up what’s been answered in the past, and also to record contact details—and add that S3 bucket to your Terraform scripts and deploy with GitHub Actions.

Do everything back to front so that the digital twin that you have deployed to production on your public URL is a fully functional, powerful digital twin. And you can share that repo so that everyone can see your expertise. And if you have any troubles doing that, particularly adding in that extra functionality, then message me. That’s what I’m there for, particularly when it comes to coding—that’s where I can really help quickly. So do get in touch. And when you successfully do this and you deploy your digital twin, whether or not you’ve added more functionality, please post about it on LinkedIn—get people excited about it. And if you tag me and say, “Look at my digital twin,” I will weigh in, I will jump in and I will try it out, of course, and then I’ll weigh in with some comments. And that way it amplifies what you’ve done around the community and to other people that I’m connected with, and generally on LinkedIn helps shine a spotlight on your achievements and your expertise.
